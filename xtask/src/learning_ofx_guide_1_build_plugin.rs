...
pub fn command() -> clap::Command {
    clap::Command::new("learning-ofx-guide-1-build-plugin")
        .about(
            "Builds and bundles the OpenFX plugin, which is then output to `crates/learning/ofx-guide-1/build`.",
        )
        .arg(
...
fn get_info_plist() -> plist::Value {
    ...
    info_plist_contents.insert(
        ...
        plist::Value::from("umajho.openfx-playground.learning.guide-1"),
    );
    info_plist_contents.insert(
...
fn build_plugin_for_target(target: &Target, release_mode: bool) -> std::io::Result<PathBuf> {
    ...
    let mut cargo_args: Vec<_> = vec![
        String::from("build"),
        String::from("--package=ofx-guide-1-basic-machinery"),
        String::from("--lib"),
        String::from("--target"),
    ...
    let mut built_library_path =
        target_dir_path.plus(target.library_prefix.to_owned() + "ofx_guide_1_basic_machinery");
...
pub fn main(args: &clap::ArgMatches) -> Result<(), Box<dyn Error>> {
    ...
    let plugin_bundle_path = output_dir.plus_iter(["OfxGuide1.ofx.bundle", "Contents"]);
    let plugin_bin_path = plugin_bundle_path.plus_iter([ofx_architecture, "OfxGuide1.ofx"]);
    ...
    if ofx_architecture == "MacOS" {
        ...
        fs::copy(
            crate_dir().plus_iter(["assets", "macos_icon.png"]),
            plugin_resources_path.plus("org.openeffects.BasicExamplePlugin.png"),
        )?;
    } else {
        fs::copy(
            crate_dir().plus_iter(["assets", "icon.png"]),
            plugin_resources_path.plus("org.openeffects.BasicExamplePlugin.png"),
        )?;
    }
