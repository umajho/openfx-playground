...
const COMMAND_NAME: &str = "learning-ofx-guide-2-build-plugin";
const PLUGIN_NAME: &str = "ofx-guide-2";
const PACKAGE_NAME: &str = "ofx-guide-2-invert";
const LIBRARY_NAME: &str = "ofx_guide_2_invert";
const BUNDLE_NAME: &str = "OfxGuide2";
const ICON_FILE_NAME: &str = "org.openeffects.InvertExamplePlugin";

static CRATE_DIR: OnceLock<PathBuf> = OnceLock::new();
fn crate_dir() -> &'static PathBuf {
    CRATE_DIR.get_or_init(|| {
        workspace_dir()
            .plus_iter(["crates", "learning", "ofx-guide-2-invert"])
            .to_path_buf()
    })
}
...
fn get_info_plist() -> plist::Value {
    build_plugin_common::build_info_plist(
        &build_plugin_common::Info {
            bundle_identifier: "umajho.openfx-playground.learning.guide-2",
