...
const COMMAND_NAME: &str = "messing-1-build-plugin";
const PLUGIN_NAME: &str = "messing-1";
const PACKAGE_NAME: &str = "messing-1-inspect-info";
const LIBRARY_NAME: &str = "messing_1_inspect_info";
const BUNDLE_NAME: &str = "Messing1";
const BUNDLE_IDENTIFIER: &str = "umajho.openeffects.messing.inspect-info";

static CRATE_DIR: OnceLock<PathBuf> = OnceLock::new();
fn crate_dir() -> &'static PathBuf {
    CRATE_DIR.get_or_init(|| {
        workspace_dir()
            .plus_iter(["crates", "messing", "messing-1-inspect-info"])
            .to_path_buf()
    })
}
