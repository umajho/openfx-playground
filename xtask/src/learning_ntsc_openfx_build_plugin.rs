...
pub fn command() -> clap::Command {
    clap::Command::new("learning-ntsc-openfx-build-plugin")
        .about(
            "Builds and bundles the NTSC OpenFX plugin, which is then output to `crates/learning/ntsc-openfx/build`.",
        )
        .arg(
        ...
        .arg(
            clap::Arg::new("destdir")
                .long("destdir")
                .help("The directory that the NTSC OpenFX plugin bundle will be output to")
...
fn build_plugin_for_target(target: &Target, release_mode: bool) -> std::io::Result<PathBuf> {
    ...
    let mut cargo_args: Vec<_> = vec![
        String::from("build"),
        String::from("--package=ntsc-openfx"),
        String::from("--lib"),
        String::from("--target"),
    ...
    let mut built_library_path =
        target_dir_path.plus(target.library_prefix.to_owned() + "ntsc_openfx");
...
fn crate_dir() -> &'static PathBuf {
    CRATE_DIR.get_or_init(|| {
        workspace_dir()
            .plus_iter(["crates", "learning", "ntsc-openfx"])
            .to_path_buf()
    })
}
...
pub fn main(args: &clap::ArgMatches) -> Result<(), Box<dyn Error>> {
    ...
    if ofx_architecture == "MacOS" {
        ...
        fs::copy(
            crate_dir().plus_iter(["vendor", "ntsc-rs", "assets", "macos_icon_less_detail.png"]),
            plugin_resources_path.plus("wtf.vala.NtscRs.png"),
        )?;
    } else {
        fs::copy(
            crate_dir().plus_iter(["vendor", "ntsc-rs", "assets", "icon.png"]),
            plugin_resources_path.plus("wtf.vala.NtscRs.png"),
        )?;
    }
