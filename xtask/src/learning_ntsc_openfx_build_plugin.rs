...
const COMMAND_NAME: &str = "learning-ntsc-openfx-build-plugin";
const PLUGIN_NAME: &str = "NTSC";
const PACKAGE_NAME: &str = "ntsc-openfx";
const LIBRARY_NAME: &str = "ntsc_openfx";
const BUNDLE_NAME: &str = "NtscRs";
const ICON_FILE_NAME: &str = "wtf.vala.NtscRs";
...
fn get_info_plist() -> plist::Value {
    build_plugin_common::build_info_plist(
        &build_plugin_common::Info {
            bundle_identifier: "rs.ntsc.openfx",
            human_readable_copyright: "© 2023-2025 valadaptive",
        },
        crate_dir(),
    )
}
...
pub fn main(args: &clap::ArgMatches) -> Result<(), Box<dyn Error>> {
    ...
    build_plugin_common::build(
        ...
        get_info_plist(),
        crate_dir().plus_iter(["vendor", "ntsc-rs", "assets"]),
        BUNDLE_NAME,
        ICON_FILE_NAME,
    )
}
