...
fn action_render(
    ...
    let output_img_m = s_ifx.clip_get_image_managed(my_data.output_clip, time, None)?;
    let source_img_m = s_ifx.clip_get_image_managed(my_data.source_clip, time, None)?;
